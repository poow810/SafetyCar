...
def is_aligned_eye(left_eye_y, right_eye_y, left_ankle_y, right_ankle_y, left_knee_y, right_knee_y):
                ...
                if keypoint_data.size(1) == 17:
                    ...
                    if is_falling:
                        if tracking_id not in tracking_data:
                            tracking_data[tracking_id] = {'start_time': current_time}
                        else:
                            elapsed_time = current_time - tracking_data[tracking_id]['start_time']
                            if elapsed_time >= 5:
                                print(f"ID: {tracking_id} - 쓰러진 상태가 5초 이상 지속됨")
                                status = "Falling"
                                color = (0, 0, 255)  # Falling일 때 빨간색
                            else:
                                status = "Falling (not confirmed)"
                                color = (0, 255, 255)  # 확인되지 않은 상태는 노란색
                    else:
                        if tracking_id in tracking_data:
                            del tracking_data[tracking_id]
                        status = "Standing"
                        color = (255, 0, 0)
