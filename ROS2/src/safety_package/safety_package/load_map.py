...
class loadMap(Node):
    ...
    def __init__(self):
        ...
        self.map_pub2 = self.create_publisher(OccupancyGrid, 'map2', 1)
        time_period=1
        ...
        pkg_path = os.getcwd()
        back_floder = 'src\\safety_package\\'
        folder_name = 'map'

        file_name = 'map.txt'
        file_name2 = 'map2.txt'
        full_path=os.path.join(pkg_path, back_floder, folder_name, file_name)
        ...
        for y in range(self.map_size_y):
            for x in range(self.map_size_x):
                if grid[x][y] == 100 :
                    ...
                    for c in range(-5, 6) :
                        for r in range(-5, 6) :
                            if 0 <= x+r < self.map_size_x and 0 <= y+c < self.map_size_y and grid[x+r][y+c] < 80 :
                                grid[x+r][y+c] = 127
