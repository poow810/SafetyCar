...
def main(x, config, goal, ob):
    ...
    while True:
        ...
        # u가 변하지 않음
        # print(f"predicted: u-{u} , trajectory-{predicted_trajectory}")
        x = motion(x, u, config.dt)
        trajectory = np.vstack((trajectory, x))
        print(f"coordi: {x[0]}, {x[1]}")
...
if __name__ == '__main__':
    # Example of how to call main with arguments
    x = np.array([0.0, 0.0, math.pi / 8.0, 0.0, 0.0])  # initial state
    config = Config()
    goal = np.array([10.0, 10.0])  # goal position
    ob = np.array([[5, 5], [3, 6], [7, 8]])  # obstacle positions
    main(x, config, goal, ob)
